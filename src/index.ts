...
function generateCspTemplates(
	...
	const contentSecurityPolicyTemplates = generateCspTemplate(
		cspConfig,
		cspRules,
		nonceConfig,
	);
		...
		(template: ContentSecurityPolicyTemplate) => {
			return {
				...
				headers: nextSafe
					...
					.default({ ...template })
					.filter(
						(header: { key: string }) => !keysToRemove.includes(header.key),
					) as Header[],
			};
		},
	);
...
export function generateSecurityHeaders(
	...
	const templates = generateCspTemplates(
		cspConfig,
		cspRules,
		keysToRemove,
		nonceConfig,
